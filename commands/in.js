...
const { getDistrictByNameAndType } = require("landkreise-deutschland");
...
exports.run = async (client, message, args) => {
    ...
    if (city == true && county == true) {
        ...
        citycheck.on("collect", async r => {
            ...
            district = getDistrictByNameAndType(`${args[0]}`, `Stadt`);
            let GetCity = async () => {
                let response = await fetch(`https://api.corona-zahlen.org/districts/${district.AGS}`)
                let In = await response.json()
                return In
            ...
            embed.addFields(
                { name: 'Inzidenz', value: someint.toFixed(2) },
                { name: 'Quelle', value: `${InValue.meta.source}` })
            embed.setFooter('📈 Diagram 🗺️ Heatmap')
            embed.setTimestamp();
        ...
        countycheck.on("collect", async r => {
            ...
            district = getDistrictByNameAndType(`${args[0]}`, `Landkreis`);
            let getIn = async () => {
                let response = await fetch(`https://api.corona-zahlen.org/districts/${district.AGS}`)
                let In = await response.json()
                return In
